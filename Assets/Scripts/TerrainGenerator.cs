...
public class TerrainGenerator : MonoBehaviour
{
    ...
    void Start() {
        ...
        chunksVisibleInViewDst = Mathf.RoundToInt(maxViewDst / meshWorldmeshWorldSize);
    ...
    void UpdateVisibleChunks() {
        ...
        for (int i = visibleTerrainChunks.Count-1; i >= 0; i--) {
            alreadyUpdatedChunksCoords.Add(visibleTerrainChunks[i].coord);
                ...
                if (!alreadyUpdatedChunksCoords.Contains(viewedChunkCoord)) {
                    if (terrainChunkDictionary.ContainsKey (viewedChunkCoord)) {
                        ...
                        TerrainChunk newChunk = new TerrainChunk(viewedChunkCoord, heightMapSettings, meshSettings, detailLevels,
                                                                colliderLODIndex, transform, mapMaterial, viewer);
